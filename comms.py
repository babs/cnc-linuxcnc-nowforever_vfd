#!/usr/bin/env python
#
#   Modified by Zeeshan Munir [2014]
#
#   comms.py is a modified version of MinimalModbus to include a custom serial protocol
#   Drives such as Sumitomo HF-430, Hitachi L300P, Hitachi SJ300 use a custom serial protocol
#   By setting the mode to MODE_CUSTOM, communication with these drives becomes feasible
...
#
#   See vfd.py to see how to communicate with the drives.
#
#   ----------------------------------------------------------------
#
#   Original Credits for MinimalModbus:
...
class Instrument():
    ...
    def _performCommand(self, functioncode, payloadToSubordinate):
        ...
        #print "payloadToSubordinate:"
        #print " ".join("%.2x"%ord(n) for n in payloadToSubordinate)
        ...
        if self.debug:
            print "message:"
            print "=>", " ".join("%.2x"%ord(n) for n in message)
        ...
        #print number_of_bytes_to_read
        # Communicate
        response = self._communicate(message, number_of_bytes_to_read)
        if self.debug:
            print "response:"
            print "<=", " ".join("%.2x"%ord(n) for n in response)
...
def _embedPayload(subordinateaddress, mode, functioncode, payloaddata):
    ...
    if mode == MODE_ASCII:
        ...
        message = _CUSTOM_HEADER + \
                _hexencode(_numToOneByteString(subordinateaddress) + _numToOneByteString(functioncode)) + \
                payloaddata + \
                _hexencode(_calculateBccString(firstPart)) + \
                _CUSTOM_FOOTER
    else:
...
def _extractPayload(response, subordinateaddress, mode, functioncode):
    ...
    BYTEPOSITION_FOR_CUSTOM_HEADER         = 0  # Relative to plain response
    ...
    # Validate the CUSTOM header and footer.
    if mode == MODE_CUSTOM:
        if response[BYTEPOSITION_FOR_CUSTOM_HEADER] != _CUSTOM_HEADER:
            raise ValueError('Did not find header ({!r}) as start of ASCII response. The plain response is: {!r}'.format( \
                _CUSTOM_HEADER,
                response))
        elif response[-len(_CUSTOM_FOOTER):] != _CUSTOM_FOOTER:
            raise ValueError('Did not find footer ({!r}) as end of ASCII response. The plain response is: {!r}'.format( \
                _CUSTOM_FOOTER,
                response))
        # Strip CUSTOM header and footer
	#print "Original Response Size:" + str(len(response))
    ...
    if mode == MODE_ASCII:
        ...
        calculateChecksum = _calculateBccString
        numberOfChecksumBytes = NUMBER_OF_BCC_BYTES
    else:
    ...
    if mode == MODE_ASCII:
        receivedChecksum = _hexdecode(response[-numberOfChecksumBytes:])   
    else:
        receivedChecksum = response[-numberOfChecksumBytes:]

    calculatedChecksum = calculateChecksum(responseWithoutChecksum)

    #print " ".join("%.2x"%ord(n) for n in receivedChecksum)
    #print " ".join("%.2x"%ord(n) for n in response[0 : len(response) - numberOfChecksumBytes])
    #print " ".join("%.2x"%ord(n) for n in calculatedChecksum)
    ...
    # Check function code
    if mode == MODE_CUSTOM:
        if functioncode == 6: #inquire about single register
            #check for ACK
            if(response[2:3] == '\x06'):
                #print "ACK Received..."
                response = response[:2]+response[3:]
	    else:
		_checkNack(_hexdecode(response[3:5]))
	elif functioncode == 3: #read data fully
	    if(response[2:3] != '\x15'):
		"""
		print "out freq:"
		print "".join(chr(ord(n)) for n in response[0*8+2:8*0+8+2])
		print " ".join("%.2x"%ord(n) for n in response[0*8+2:8*0+8+2])
		print "out current:"
		print "".join(chr(ord(n)) for n in response[1*8+2:8*1+8+2])
		print "direction of rotation:"
		print "".join(chr(ord(n)) for n in response[2*8+2:8*2+8+2])
		print "output torque monitor:"
		print "".join(chr(ord(n)) for n in response[7*8+2:8*7+8+2])
		print "output voltage monitor:"
		print "".join(chr(ord(n)) for n in response[8*8+2:8*8+8+2])
		print "electric power monitor:"
		print "".join(chr(ord(n)) for n in response[9*8+2:8*9+8+2])
		print "run time:"
		print "".join(chr(ord(n)) for n in response[11*8+2:8*11+8+2])
		print " ".join("%.2x"%ord(n) for n in response[11*8+2:8*11+8+2])
		print "power on time:"
		print "".join(chr(ord(n)) for n in response[12*8+2:8*12+8+2])
		print " ".join("%.2x"%ord(n) for n in response[12*8+2:8*12+8+2])
		"""
	    else:
	        _checkNack(_hexdecode(response[3:5]))
	if functioncode == 1: #set the frequency
	    if(response[2:3] != '\x06'):
		_checkNack(_hexdecode(response[3:5]))
        if functioncode == 0: #stop/start/run
            if(response[2:3] != '\x06'):
                _checkNack(_hexdecode(response[3:5]))

		
			
        # Convert the CUSTOM (stripped) response string to RTU-like response string
        #print " ".join("%.2x"%ord(n) for n in response[0 : len(response) - numberOfChecksumBytes])
        #response = _hexdecode(response)
	#print "data received:"
        #print " ".join("%.2x"%ord(n) for n in response)
    ...
    if mode == MODE_CUSTOM:
	responseaddress = ord(_hexdecode(response[0:2]))
    else:
    ...
    if mode == MODE_ASCII:
        ...
        lastDatabyteNumber = len(response) - NUMBER_OF_BCC_BYTES
    else:
    ...
    if mode == MODE_ASCII:
        payload = "".join(chr(ord(n)) for n in payload)
    #print "payload converted:"
    #print "".join(chr(ord(n)) for n in payload)
    return payload
...
def _predictResponseSize(mode, functioncode, payloadToSubordinate):
    ...
    NUMBER_OF_CUSTOM_RESPONSE_STARTBYTES = 3
    NUMBER_OF_CUSTOM_RESPONSE_ENDBYTES = 3
    ...
    if mode != MODE_CUSTOM:
        ...
        if functioncode in [6]:
            response_payload_size = 9
	if functioncode in [3]:
	    response_payload_size = 104
    ...
    if mode == MODE_CUSTOM:
        return NUMBER_OF_CUSTOM_RESPONSE_STARTBYTES + \
            response_payload_size + \
            NUMBER_OF_CUSTOM_RESPONSE_ENDBYTES
    else:
...
def _calculateBccString(inputstring):
    """Calculate BCC for custom communication protocols
    ...
    Returns:
        A one-byte BCC bytestring (not encoded to hex-string)
    """
    _checkString(inputstring, description='input BCC string')

    register = 0
    for character in inputstring:
        register ^= ord(character)

    bccString = _numToOneByteString(register)
    ...
    for i in range(0,len(inputstring)/8):
        print int(inputstring[i*8:i*8+8],base=2)
        lastval ^= int(inputstring[i*8:i*8+8],base=2)
        print lastval

    #print lastval
    bccString = _numToOneByteString(lastval)
    '''
    return bccString
...
def _checkMode(mode):
    ...
    if mode not in [MODE_RTU, MODE_ASCII, MODE_CUSTOM]:
...
def _checkNack(nackvalue):
    """Determine the cause of NACK (Negative Acknowledge)
    ...
    """
    if nackvalue == '\x01':
	raise ValueEror('NACK Recieved... Error: 01H Parity Error')
    elif nackvalue == '\x02':
	raise ValueError('NACK Recieved... Error: 02H Check Sum Error')
    elif nackvalue == '\x03':
	raise ValueError('NACK Recieved... Error: 03H Framing Error')
    elif nackvalue == '\x04':
	raise ValueError('NACK Recieved... Error: 04H Overrun Error')
    elif nackvalue == '\x05':
	raise ValueError('NACK Recieved... Error: 05H Protocol Error')
    elif nackvalue == '\x06':
	raise ValueError('NACK Recieved... Error: 06H ASCII Code Error')
    elif nackvalue == '\x07':
	raise ValueError('NACK Recieved... Error: 07H Receive Buffer Overrun Error')
    elif nackvalue == '\x08':
	raise ValueError('NACK Recieved... Error: 08H Receive Time-out Error')
    elif nackvalue == '\x11':
	print 'NACK Recieved... Error: 09H Abnormal Command Code Error'
    elif nackvalue == '\x13':
	raise ValueError('NACK Recieved... Error: 13H Test Code Error')
    elif nackvalue == '\x16':
	print 'NACK Recieved... Error: 16H Abnormal Parameter Code/Value Error'
    else:
	raise ValueError('NACK Recieved... Unknown Error')
